...
const INJECTED_STYLES = `
  ...
  .text-silver-matte {
      background: linear-gradient(180deg, var(--color-foreground) 0%, color-mix(in srgb, var(--color-foreground) 80%, transparent) 100%);
...
export function CinematicHero({ 
  ...
  cardDescription = <><span className="text-white font-semibold">Zink AI Minutes</span> is the only AI note-taker built for mixed English &amp; Shona conversations—capturing every word, across every device, in real time.</>,
  metricValue = 120,
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      scrollTl
        .to([".hero-text-wrapper", ".bg-grid-theme"], { scale: 1.1, filter: "blur(16px)", opacity: 0.2, ease: "power2.inOut", duration: 1 }, 0)
        ...
        .fromTo(".phone-widget", { y: 30, autoAlpha: 0, scale: 0.95 }, { y: 0, autoAlpha: 1, scale: 1, stagger: 0.1, ease: "back.out(1.2)", duration: 0.8 }, "-=0.8")
        .to(".progress-ring", { strokeDashoffset: 60, duration: 1, ease: "power3.inOut" }, "-=0.7")
        .to(".counter-val", { innerHTML: metricValue, snap: { innerHTML: 1 }, duration: 1, ease: "expo.out" }, "-=1.0")
        .fromTo(".floating-badge", { y: 60, autoAlpha: 0, scale: 0.8, rotationZ: -6 }, { y: 0, autoAlpha: 1, scale: 1, rotationZ: 0, ease: "back.out(1.5)", duration: 0.8, stagger: 0.12 }, "-=0.9")
        .fromTo(".card-left-text", { x: -40, autoAlpha: 0 }, { x: 0, autoAlpha: 1, ease: "power4.out", duration: 0.8 }, "-=0.7")
        .fromTo(".card-right-text", { x: 40, autoAlpha: 0, scale: 0.85 }, { x: 0, autoAlpha: 1, scale: 1, ease: "expo.out", duration: 0.8 }, "<")
        .to({}, { duration: 0.3 })
        .set(".hero-text-wrapper", { autoAlpha: 0 })
        .set(".cta-wrapper", { autoAlpha: 1 })
        .to({}, { duration: 0.2 })
        .to([".mockup-scroll-wrapper", ".floating-badge", ".card-left-text", ".card-right-text"], {
          scale: 0.92, y: -30, z: -150, autoAlpha: 0, ease: "power3.in", duration: 0.7, stagger: 0.04,
        })
        .to(".main-card", { 
        ...
        .to(".cta-wrapper", { scale: 1, filter: "blur(0px)", ease: "expo.inOut", duration: 1.0 }, "pullback")
        .to(".main-card", { y: -window.innerHeight - 200, ease: "power3.in", duration: 0.8 });
  ...
  return (
    ...
    >
      ...
      <div className="hero-text-wrapper absolute z-10 flex flex-col items-center justify-center text-center w-screen px-4 will-change-transform" style={{ overflow: 'visible' }}>
        <h1 className="text-track gsap-reveal text-3d-matte text-5xl md:text-7xl lg:text-[6rem] font-bold tracking-tight mb-2">
          {tagline1}
        </h1>
        <h1 className="text-days gsap-reveal text-silver-matte text-5xl md:text-7xl lg:text-[6rem] font-extrabold tracking-tighter" style={{ overflow: 'visible' }}>
          {tagline2}
        </h1>
      </div>
      ...
      <div className="cta-wrapper absolute z-10 flex flex-col items-center justify-center text-center w-screen px-4 gsap-reveal pointer-events-auto will-change-transform">
        ...
        <div className="flex flex-col sm:flex-row gap-6">
          ...
          {/* Google Play */}
          <a href={googlePlayUrl} target="_blank" rel="noopener noreferrer" aria-label="Get it on Google Play" className="btn-modern-dark flex items-center justify-center gap-3 px-8 py-4 rounded-[1.25rem] group focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-background">
          ...
          {/* Microsoft Store */}
          <a href={microsoftStoreUrl} target="_blank" rel="noopener noreferrer" aria-label="Get it from Microsoft Store" className="btn-modern-light flex items-center justify-center gap-3 px-8 py-4 rounded-[1.25rem] group focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
            <svg className="w-7 h-7 transition-transform group-hover:scale-105" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M11.5 2.75h-9v9h9v-9zm0 10.5h-9v9h9v-9zm1 0v9h9v-9h-9zm0-10.5v9h9v-9h-9z"/>
            </svg>
            <div className="text-left">
              <div className="text-[10px] font-bold tracking-wider text-neutral-500 uppercase mb-[-2px]">Get it from</div>
              <div className="text-xl font-bold leading-none tracking-tight">Microsoft</div>
            </div>
          </a>
        </div>
      </div>
      ...
      <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none" style={{ perspective: "1500px" }}>
        ...
        >
          ...
          <div className="relative w-full h-full max-w-7xl mx-auto px-4 lg:px-12 flex flex-col justify-evenly lg:grid lg:grid-cols-3 items-center lg:gap-8 z-10 py-6 lg:py-0">
            ...
            {/* iPhone Mockup: MIDDLE (Mobile) / CENTER (Desktop) */}
            <div className="mockup-scroll-wrapper order-2 lg:order-2 relative w-full h-[380px] lg:h-[600px] flex items-center justify-center z-10" style={{ perspective: "1000px" }}>
              <div className="relative w-full h-full flex items-center justify-center transform scale-[0.65] md:scale-85 lg:scale-100">
                    ...
                    <div className="relative w-full h-full pt-12 px-5 pb-8 flex flex-col">
                      <div className="phone-widget flex justify-between items-center mb-8">
                        <div className="flex flex-col">
                          <span className="text-[10px] text-neutral-400 uppercase tracking-widest font-bold mb-1">Recording</span>
                          <span className="text-xl font-bold tracking-tight text-white drop-shadow-md">Meeting</span>
                        </div>
                      ...
                      <div className="phone-widget relative w-44 h-44 mx-auto flex items-center justify-center mb-8 drop-shadow-[0_15px_25px_rgba(0,0,0,0.8)]">
                        <svg className="absolute inset-0 w-full h-full" aria-hidden="true">
                          ...
                          <circle className="progress-ring" cx="88" cy="88" r="64" fill="none" stroke="#F5A623" strokeWidth="12" />
                        </svg>
                ...
                <div className="floating-badge absolute flex top-6 lg:top-12 left-[-15px] lg:left-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">Live Transcription</p>
                    <p className="text-amber-200/50 text-[10px] lg:text-xs font-medium">English &amp; Shona</p>
                  </div>
                </div>
                ...
                <div className="floating-badge absolute flex bottom-12 lg:bottom-20 right-[-15px] lg:right-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">Notes Exported</p>
                    <p className="text-emerald-200/50 text-[10px] lg:text-xs font-medium">PDF &amp; Word ready</p>
                  </div>
                </div>
              </div>
            </div>

            {/* Left Text: BOTTOM (Mobile) / LEFT (Desktop) */}
